...
### Added
- **An unreachable tingbok no longer has to mean a vocabulary-less parse** — where the `tingbok` package itself is installed (the new `tingbok` extra, and by definition on the machine that runs the service), the vocabulary, the batch resolve, the ancestor chains and the source registry are answered in-process instead, through tingbok's new `tingbok.embedded` entry point. Strictly a fallback: HTTP is tried first and a reachable tingbok is never second-guessed. An in-process answer is never written to the client-side cache, so — unlike a fetched one, which is held for 60 days — it can never suppress a later real request. Read-only, deliberately — an EAN observation is a write to the service's data file, and writing it into a local copy would create a divergence nobody asked for, so a failed push stays a failed push. With no tingbok installed, or one older than `tingbok.embedded`, the import fails once, is remembered as unavailable, and everything behaves exactly as before. The resolve is asked for offline, because a caller that got here has no network and falling through to DBpedia for every unknown label would only burn the timeout again, once per label. New module `inventory_md.tingbok_embedded`.
- **The category browser's source names now come from tingbok** — new `inventory_md.sources` module, fed by tingbok's `GET /api/sources`, holding what `vocabulary.py` used to keep as a private `_SOURCE_LABELS` dict and a `_uri_to_source()` if-chain. Those were a second copy of something tingbok already knew, so adding a source to tingbok did not show up here until this package was released again. An http(s) source is identified by **host** rather than by a literal URI prefix, which also fixes what a naive port of the old table would have broken: upstream data carries `de.dbpedia.org` and `fr.dbpedia.org` (four concepts in one real inventory), `wikidata.org` with and without `www.`, and both http and https spellings, and the old code caught them by substring. The bundled table survives as the offline fallback — an inventory is routinely parsed with tingbok unreachable, and the generated `vocabulary.json` is read by a static web UI with no server at all — but it no longer has to be *updated*, and a source it has never heard of degrades to a title-cased name rather than a blank. The fetched list is cached for 60 days.
- **`is_descendant_of()` can ask tingbok when the local vocabulary cannot place a category** — "is soybeans food?" was answered by walking `broader` links locally, which returns False for a category the inventory has never resolved. Pass `tingbok_url=` and such a concept is looked up through tingbok's new `GET /api/ancestors/{id}` instead (via `fetch_ancestors_from_tingbok()`, cached for 60 days — but only when tingbok actually answered: an unreachable server is not cached at all, because a tingbok predating the endpoint 404s it and one such run would otherwise disable the feature for two months). Only for a concept the local vocabulary genuinely cannot place — one it *has* placed has an answer already, and this runs in loops over every inventory item, so second-guessing every negative would be an HTTP request apiece. An unreachable tingbok leaves the local answer standing.

### Fixed
- **`parse` no longer reports failed EAN pushes as pushed.** A PUT that tingbok rejected, or that never got an answer, was still counted in "Pushed N observation(s)". The summary now reads "Pushed N observation(s), M failed, K already up-to-date".
- **`add` and `move` into a container with sub-containers filed the item in the last sub-container** — a container's section spans its sub-headings, and the "after the last bullet" rule walked straight into them, so `location: food2` landed in `food2-lost` and newly bought food was filed as already lost, silently, for weeks. The item now goes into the container itself: after its own items, or, if it has none, after its description and before its first sub-heading.
- **Categories written as a path whose parent does not exist were invisible, not merely misfiled** — `epoxy/filler`, `epoxy/hardener` and `epoxy/pigment` were all in one real inventory with no `epoxy` concept anywhere, and hierarchy inference only links a path parent that already exists. The category tree's root list excludes anything with a "/" in its id, so all three were reachable from no root: not in the browser, not under a parent, nowhere. Nine concepts were in that state. `build_category_tree()` now creates the missing path ancestors first (new `create_path_ancestor_stubs()`), so `epoxy` appears with its three children under it — and a stub created at the top level is added to the virtual root's `narrower` whitelist, since that, not the concept dict, is what the tree's root list is built from whenever tingbok's `_root` is present, which is the normal case. Only concepts with **no** `broader` at all get this: one that declares a parent has already been placed, possibly somewhere its path does not suggest — `klær/jakke` is wired to the canonical `clothing` with no `klær` concept created — and stubbing every path prefix blindly would undo that.
- **`inventory-md-check-quality` no longer misses food that is more than one step from `food`** — the food test looked at a concept's own id and its direct `broader` only, so `soybeans` → `legumes` → `food/legumes` came back as non-food and its missing best-before went unreported. It now consults tingbok's ancestors endpoint when the shallow test says no; tingbok being unreachable, or not having the concept, leaves the old answer standing, so an offline run is no worse than before.
- **Client-side tingbok caches were expiring eight times too often** — the EAN and category-lookup caches used a 7-day TTL against tingbok's own 60, and tingbok's source carried a comment asserting the two matched. Both are now 60 days for answers tingbok *gave*. A product's identity and a concept's place in the hierarchy change about as often as tingbok gains a new upstream; the shorter figure bought nothing but traffic. A cached *absence* is different and keeps a 7-day TTL of its own: an EAN tingbok does not know about is precisely the kind that gets contributed, and holding "not found" for two months would hide someone else's contribution.

### Changed
- **`inventory-md-check-quality` reports category IDs that differ only in separator or plural form** — `cling-film` and `clingfilm`, `bike-clamp` and `bike_clamp`, `lentil` and `lentils` are each one category written two ways, and each spelling then carries half the items. A real inventory had 65 such groups. Reported at INFO and not repaired: which spelling is canonical — dashes or underscores, singular or plural — is an open question in tingbok's own consistency TODO, and normalising them here would decide it by accident. The whole ID is normalised rather than just the leaf, because upstream taxonomies reach one concept by many ancestor chains (`book` sits under 30 different paths) and those are not duplicates.
- **What `vocabulary.py` called the "package vocabulary" is now called the tingbok vocabulary** — nothing has bundled a vocabulary since it moved to tingbok, and the name misled on every read.
...
- **A photo whose barcode is present but unreadable is now reported instead of passing silently** — `extract_barcodes.py` gains `looks_like_undecoded_barcode()`, which flags an image that decoded to nothing but contains a barcode-like pattern (rows that are busy *and* near-identical to their neighbours, which is what full-height bars look like and dense text does not). Specimen: a diving-mask label (`8680041405983`) torn straight through the right-hand quiet zone that zbar requires, printed with a striated thermal texture — plainly legible to a human, no read at all from zbar, and no amount of crop/rotation retrying helps. Silence was the worst possible answer, because the photo then looked like one that simply had no barcode. It detects *presence*, not the defect: the emitted `tag:TODO` block says "torn, occluded, blurred, or missing the quiet zone — read the digits by hand and use `inventory-md ean`". `--no-scan-undecoded` turns it off.
...
### Changed
- **`check_quality.py` is now `inventory_md.check_quality` with an `inventory-md-check-quality` console script** — it was a loose script in `scripts/`, runnable only by path. purchase-pipeline's commit pipeline runs the quality gate as its final validation step, and hardcoding a path into a sibling checkout of this repo is exactly the coupling the split removes. Invoking it by name matches how the same pipeline already calls `inventory-md parse`. The quality gate itself is unchanged, and stays in this project — it validates the inventory format, which is inventory's business.
- **`bb_dates` moved from `scripts/` into the package** (`inventory_md.bb_dates`) — a pure, dependency-free date parser that was reachable only via a `sys.path` insert from sibling scripts. purchase-pipeline reads dates off receipts with the same parser `extract_barcodes.py` uses on photos, and a loose script is not something a separate distribution can depend on. Whose business it is does not change: it stays here.
...
- **Unknown-category warnings consult tingbok before crying wolf** — `add_item()` (and thus `inventory-md add` and any caller importing it) now falls back to tingbok's vocabulary when a category doesn't resolve locally, so categories merely *new to this inventory* no longer warn or fail `--strict`. When tingbok is unreachable the local verdict stands. New optional `tingbok_url` parameter, supplied from config by the CLI.

### Removed
- **The purchasing pipeline has moved to its own project, [purchase-pipeline](https://github.com/tobixen/purchase-pipeline)** — `shop_import.py`, `ledger.py`, `pipeline.py`, `shopping_context.py`, `inventory_import.py`, `staging.py`, `receipt_formats.py`, `tingbok_push.py`, `off_upload.py`, `openprices_publish.py` and `op_auth.py` are gone from `scripts/`, along with `receipt-formats.json`. They inverted the dependency: a general-purpose inventory tool should not know about one particular person's accounting, Open Food Facts account and receipt formats. Deciding what a purchase *means* is not inventory's business; identifying a physical object is, which is why `extract_barcodes.py`, `bb_dates`, `check_quality.py`, the vocabulary system and the web UI all stay. They are now console scripts (`shop-import`, `purchase-ledger`, `purchase-pipeline`, …) rather than paths under `scripts/`; purchase-pipeline depends on this project and this project knows nothing about it.
...
### Fixed
- **`extract_barcodes.py` no longer reports a phantom checksum-valid EAN as a peer read** — one photo of a Dr. Oetker vanilla sugar sachet decoded as *both* `5941132002140` (real) and `2931532002140` (phantom), and the tool printed the two as equal item lines. A valid check digit is not evidence of a correct read: EAN-13 carries its leading digit in the *parity pattern* of the left six digits rather than in bars of its own, so a couple of bar-width errors flip left-half digits into their opposite-parity twins, change the decoded leading digit, and the checksum is then recomputed over the corrupted digits and passes. Each image is now decoded three ways (original, 0.6×, autocontrast — see `DECODE_VARIANTS`), which re-quantises the bar widths and moves the binarisation threshold, and every read is located in the original image's coordinates (`bbox`). Reads whose boxes overlap are candidates for **one** symbol, and exactly one of them wins, decided in order of how much the evidence is worth: (1) GS1 prefix — a retail pack never carries a restricted-distribution or coupon prefix, which is free, offline, and settled the vanilla sachet without a single lookup; (2) product resolution — the only candidate tingbok knows; (3) corroboration, and only by a clean margin. Winning is not enough to delete the others, because sharing a place in the photo is not proof of being the same barcode — a small sticker sits on a big label, one package lies over another. A losing read is discarded (as a `# Discarded conflicting read(s):` comment under the winner) only when something explains it as a misread: the parity signature, a restricted prefix, or resolving to nothing where the winner resolves. Anything less and the photo becomes a single `tag:TODO … (needs review — conflicting barcode reads)` block listing every candidate, rather than plausible-looking lines to pick from by eye. Corroboration is deliberately *last*: on a second specimen (a Bulgarian луканка label) the phantom `3200274928780` out-corroborated the real `3800214928780` **3 reads to 2** — a majority, and wrong — which only product resolution could untangle. Spatial grouping is likewise not a digit heuristic because it need not be: a third read of that same label, `3800874988780`, shared no right half with the real code and so carried no parity signature at all, yet sat squarely on the same barcode. A product database is consulted *only* to break a conflict, never to vet a lone read — a genuinely new product is absent from it, and discarding such a read would be worse than the bug being fixed — and two barcodes in different parts of a shelf photo are two products, not a conflict. New `resolve_candidates()`, `bbox_overlap_fraction()` and a `corroborate=` parameter on `extract_barcodes()`; `--no-corroborate` restores the old single-pass scan.
- **An estimated best-before is no longer recorded as a printed one** — a best-before can be marked estimated inline (`--bb 2026-07-09:EST`) or out of band (a `bb_est` flag from a caller). The two spellings were resolved in different places and could disagree silently, writing a shelf-life guess as a bare `bb:2027-01-01` — exactly the error `:EST` exists to prevent. They are now reconciled in one place, the new `additem.resolve_bb_est()`: an explicit flag wins over the heuristic, and a `:EST` suffix contradicted by `bb_est: false` (or a non-boolean value) raises rather than picking silently. `add_item()` accepts a `:EST` suffix on `bb`, and its `bb_est` parameter grew a third state (`None` = unspecified) so `inventory-md add --bb 2026-09:EST` no longer collides with the implicit `--est` default.
...
- **`inventory-md add` no longer crashes on a YAML-native best-before date** — an unquoted `bb: 2027-02-12` in a caller's YAML reaches `add_item()` as `datetime.date` and `validate_bb_format()` raised TypeError. Date/datetime values are now coerced to their ISO string form.
