...
def vocabulary_command(args, config: Config) -> int:
    ...
    # Merge vocabulary.json from the directory (generated by parse, contains enriched
    # broader chains from the resolve endpoint).  Takes priority: parse produces the most
    # complete picture of each concept's hierarchy for this specific inventory.
    vocab_json_path = directory / "vocabulary.json"
    if vocab_json_path.exists():
        parsed_vocab = vocabulary.load_local_vocabulary(vocab_json_path)
        vocab = vocabulary.merge_vocabularies(parsed_vocab, vocab)
    ...
    elif args.vocab_command == "search":
